...
import { Banknote, TrendingUp, Percent, ShoppingBag, Boxes, Wallet, ArrowUpRight } from "lucide-react";
import { prisma } from "@/lib/prisma";
import { requireRole } from "@/lib/auth";
import { formatBDT, formatBDTCompact, formatNumber, formatDate } from "@/lib/format";
...
async function getData(tenantId: string, days: number) {
  ...
  const [orders, stockLevels] = await Promise.all([
  ...
  // Seed trend buckets for a continuous line
  ...
  for (const o of orders) {
    revenue += o.total;
    const dayKey = o.orderDate.toISOString().slice(0, 10);
    if (trendMap.has(dayKey)) trendMap.set(dayKey, (trendMap.get(dayKey) ?? 0) + o.total);
  ...
  const bestSellers = [...productAgg.values()].sort((a, b) => b.revenue - a.revenue).slice(0, 8);
  ...
  // Inventory valuation (current, not period-bound)
  let invCost = 0;
  let invRetail = 0;
  ...
  return { revenue, cogs, grossProfit, margin, orderCount, aov, units, bestSellers, trend, categoryData, invCost, invRetail };
}
...
export default async function ReportsPage({
  ...
  return (
    <div>
      <PageHeader eyebrow="Overview" title="Reports" description="Sales performance, profitability and inventory value.">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard label="Revenue" value={formatBDT(d.revenue)} hint={`${d.orderCount} orders`} icon={Banknote} tone="gold" delay={0} />
        <StatCard label="Gross profit" value={formatBDT(d.grossProfit)} hint={`Cost ${formatBDTCompact(d.cogs)}`} icon={TrendingUp} tone="success" delay={60} />
        <StatCard label="Profit margin" value={`${d.margin.toFixed(1)}%`} hint="Revenue minus cost" icon={Percent} delay={120} />
        <StatCard label="Avg order value" value={formatBDT(d.aov)} hint={`${formatNumber(d.units)} units sold`} icon={ShoppingBag} delay={180} />
      </div>
        ...
        <Card className="animate-rise" style={{ animationDelay: "220ms" }}>
          ...
          <CardContent className="space-y-4">
            ...
            <p className="text-xs text-muted-foreground">
              Potential margin in stock: {formatBDT(d.invRetail - d.invCost)}
            </p>
          </CardContent>
        </Card>
      </div>
